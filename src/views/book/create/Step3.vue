<template>
  <div class="w-150 m-auto">
    ...
    <!-- 显示图书基本信息 -->
    <div class="max-w-4xl mx-auto mt-8 px-8 py-10 bg-white rounded-xl shadow-sm border border-gray-100" v-if="bookInfo">
      ...
      <div class="bg-gray-50 rounded-lg p-6">
        <Descriptions :column="1" class="space-y-4">
          ...
          <Descriptions.Item label="目录" class="text-base">
            <div class="bg-white rounded-lg p-4 mt-2 max-h-64 overflow-y-auto">
              <ul class="space-y-2">
                <li v-for="(item, index) in bookInfo.bookMenuList" :key="index" class="flex items-start text-gray-700">
                  <span class="w-6 h-6 bg-blue-50 text-blue-600 rounded-full flex items-center justify-center text-xs font-medium mr-3 mt-0.5 flex-shrink-0">
                    {{ index + 1 }}
                  </span>
